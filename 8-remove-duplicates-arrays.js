...
function removeDuplicates(nums) {
    let x = 0; // Pointer to track the last unique element's position

    for (let i = 0; i < nums.length; i++) {
        if (nums[i] > nums[x]) { // Check if the current element is greater than the last unique element
            x = x + 1; // Move the pointer to the next position
            nums[x] = nums[i]; // Update the array with the new unique element
        }
    }
